...
type AviSession struct {
	...
	// tokenAuth specifies whether we should use an authenticatoin token
	tokenAuth bool
	...
	// internal: auth token generated by Django, for use in token mode
	authToken string
...
func NewAviSession(host string, username string, options ...func(*AviSession) error) (*AviSession, error) {
	...
	if avisess.tokenAuth {
		err := avisess.setAuthToken()
		return avisess, err
	} else {
...
func (avisess *AviSession) setAuthToken() error {
...
// SetTokenAuth - Use auth token rather than credentials
func SetTokenAuth(avisess *AviSession) error {
	avisess.tokenAuth = true
	return nil
}
...
func (avisess *AviSession) restRequest(verb string, uri string, payload interface{}) ([]byte, error) {
	...
	if resp.StatusCode == 401 && avisess.tokenAuth {
		// auth token incorrect; get new auth token and retry the request
		avisess.setAuthToken()
		return avisess.restRequest(verb, uri, payload)
	}
